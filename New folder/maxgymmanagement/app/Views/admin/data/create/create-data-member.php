         ...
         <div class="col-lg-12 col-md-12">
            <div class="card">
               ...
               <div class="card-body mx-5 my-3">
                  ...
                  <form action="<?= base_url('admin/member/create'); ?>" method="post" enctype="multipart/form-data">
                     <?= csrf_field() ?>
                     <?php $validation = \Config\Services::validation(); ?>

                     <div class="form-group mt-4">
                        <label for="nama">Nama Member</label>
                        <input type="text" id="nama" class="form-control <?= $validation->getError('nama') ? 'is-invalid' : ''; ?>" name="nama" placeholder="Nama Member" value="<?= old('nama') ?? $oldInput['nama'] ?? '' ?>" required>
                     ...
                     <div class="form-group mt-2">
                        ...
                        if (old('jenis_kelamin')) {
                           $l = (old('jenis_kelamin') ?? $oldInput['jenis_kelamin'] ?? '') == 'Laki-laki' ? 'checked' : '';
                           $p = (old('jenis_kelamin') ?? $oldInput['jenis_kelamin'] ?? '') == 'Perempuan' ? 'checked' : '';
                        }
                        ?>
                        <div class="form-check form-control pt-0 mb-1 <?= $validation->getError('jenis_kelamin') ? 'is-invalid' : ''; ?>">
                           <div class="row">
                              ...
                              <div class="col">
                                 <div class="row">
                                    <div class="col-auto pr-1">
                                       <input class="form-check" type="radio" name="jenis_kelamin" id="perempuan" value="Perempuan" <?= $p ?? ''; ?>>
                                    </div>
                                    <div class="col">
                     ...
                     <div class="form-group mt-4">
                        ...
                        <input type="text" id="no_member" class="form-control <?= $validation->getError('no_member') ? 'is-invalid' : ''; ?>" name="no_member" placeholder="No Member" value="<?= old('no_member') ?? $oldInput['no_member'] ?? '' ?>" required>
                        <div class="invalid-feedback">
                     ...
                     <div class="form-group mt-2">
                        ...
                        <?php
                        $u = (old('type') ?? $oldInput['type'] ?? '') == 'umum' ? 'checked' : '';
                        $p = (old('type') ?? $oldInput['type'] ?? '') == 'pelajar' ? 'checked' : '';
                        $m = (old('type') ?? $oldInput['type'] ?? '') == 'mahasiswa' ? 'checked' : '';
                        $pt = (old('type') ?? $oldInput['type'] ?? '') == 'personal_trainer' ? 'checked' : '';
                        $mpt = (old('type') ?? $oldInput['type'] ?? '') == 'member_pt' ? 'checked' : '';
                        ?>
                     ...
                     <div class="form-group mt-4">
                        ...
                        <input type="email" id="email" name="email" class="form-control <?= $validation->getError('email') ? 'is-invalid' : ''; ?>" placeholder="email@example.com" value="<?= old('email') ?? $oldInput['email'] ?? '' ?>">
                        <div class="invalid-feedback">
                     ...
                     <div class="form-group mt-4">
                        ...
                        <textarea id="alamat" name="alamat" class="form-control" rows="3"><?= old('alamat') ?? $oldInput['alamat'] ?? '' ?></textarea>
                     </div>

                     <div class="form-group mt-4">
                        <label for="tanggal_join">Tanggal Join</label>
                        <input type="date" id="tanggal_join" name="tanggal_join" class="form-control <?= $validation->getError('tanggal_join') ? 'is-invalid' : ''; ?>" value="<?= old('tanggal_join') ?? $oldInput['tanggal_join'] ?? '' ?>" required>
                        <div class="invalid-feedback">
                     ...
                     <div class="form-group mt-4">
                        ...
                        <input type="date" id="tanggal_expired" name="tanggal_expired" class="form-control <?= $validation->getError('tanggal_expired') ? 'is-invalid' : ''; ?>" value="<?= old('tanggal_expired') ?? $oldInput['tanggal_expired'] ?? '' ?>" required>
                        <div class="invalid-feedback">
                     ...
                     <div class="form-group mt-4">
                        ...
                        <textarea id="keterangan" name="keterangan" class="form-control" rows="3"><?= old('keterangan') ?? $oldInput['keterangan'] ?? '' ?></textarea>
                     </div>
